    ...
    <div className="min-h-screen bg-gradient-to-b from-white via-blue-50/30 to-white flex flex-col">
      ...
      <main className="flex-1 flex flex-col max-w-4xl w-full mx-auto px-4 pt-32 pb-6">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4" style={{ maxHeight: '55vh' }}>
            {messages.map((msg) => (
              <div key={msg.id} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'} gap-3`}>
                {msg.sender === 'bot' && (
                  <div className="w-8 h-8 rounded-full bg-gradient-to-br from-teal-500 to-emerald-500 flex items-center justify-center flex-shrink-0 mt-1">
              ...
              <div className="flex flex-wrap gap-2">
                {SUGGESTED_QUESTIONS.map((q, i) => (
                  <button
                    ...
                    className="px-3 py-1.5 text-xs bg-teal-50 border border-teal-200 text-teal-700 rounded-full hover:bg-teal-100 transition font-medium"
                  >
                    {q}
                  </button>
                ))}
            ...
            >
              <div className="flex-1 relative">
                <input
                  ...
                  disabled={isTyping}
                  className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:outline-none focus:border-teal-500 transition text-sm bg-white disabled:opacity-60"
                  onKeyDown={(e) => {
              ...
              <button
                ...
                className="w-11 h-11 flex items-center justify-center bg-gradient-to-br from-teal-600 to-emerald-600 text-white rounded-xl hover:from-teal-700 hover:to-emerald-700 transition shadow-md disabled:opacity-50 disabled:cursor-not-allowed flex-shrink-0"
              >
        ...
        <div className="mt-4 grid grid-cols-3 gap-3">
          <Link
            to="/analyze"
            className="flex flex-col items-center gap-1 p-3 bg-white border border-gray-200 rounded-xl hover:border-teal-500 hover:bg-teal-50 transition text-center group"
          >
            <FlaskConical className="w-5 h-5 text-teal-600" />
            <span className="text-xs font-semibold text-gray-700 group-hover:text-teal-700">Analyze Skin</span>
          </Link>
          <Link
            to="/hospitals"
            className="flex flex-col items-center gap-1 p-3 bg-white border border-gray-200 rounded-xl hover:border-teal-500 hover:bg-teal-50 transition text-center group"
          >
            ...
            <span className="text-xs font-semibold text-gray-700 group-hover:text-teal-700">Find a Clinic</span>
          </Link>
          <Link
            to="/resources"
            className="flex flex-col items-center gap-1 p-3 bg-white border border-gray-200 rounded-xl hover:border-teal-500 hover:bg-teal-50 transition text-center group"
          >
            ...
            <span className="text-xs font-semibold text-gray-700 group-hover:text-teal-700">Learn More</span>
          </Link>
        </div>
      </main>
    </div>
